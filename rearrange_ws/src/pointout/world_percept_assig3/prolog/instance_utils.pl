...
get_class(Class):-
	getClassPath(Class, ClassPath), % get the path of the class
	( rdf_has(ClassPath, rdf:type, owl:'Class') ->
        % print it is existed, if the class existed
		nl,
        fail % Force the predicate to fail if the class already exists
    ;   % If the class does not exist, assert it and report
        rdf_assert(ClassPath, rdf:type, owl:'Class'),
        write('New class created: '), write(ClassPath), nl
    ).
